...
internal sealed class CSharpRegionSnippetProvider() : AbstractSingleChangeSnippetProvider<RegionDirectiveTriviaSyntax>
{
    ...
    protected override ValueTask<RegionDirectiveTriviaSyntax> AdjustSnippetExpressionAsync(
        Document document, RegionDirectiveTriviaSyntax node, CancellationToken cancellationToken)
    {
        // The base class adds elastic trivia to inner tokens so the formatter can
        // normalise whitespace. For preprocessor directives this is harmful because
        // the formatter may move the directive away from the cursor position where
        // the user invoked the snippet. Strip the elastic markers so the directive
        // stays exactly where it was inserted.
        var cleanNode = node.ReplaceTokens(
            node.DescendantTokens(descendIntoTrivia: true),
            (_, rewritten) =>
            {
                rewritten = rewritten.WithoutAnnotations(SyntaxAnnotation.ElasticAnnotation);
                rewritten = rewritten.WithLeadingTrivia(
                    rewritten.LeadingTrivia.Where(t => !t.HasAnnotation(SyntaxAnnotation.ElasticAnnotation)));
                rewritten = rewritten.WithTrailingTrivia(
                    rewritten.TrailingTrivia.Where(t => !t.HasAnnotation(SyntaxAnnotation.ElasticAnnotation)));
                return rewritten;
            });
