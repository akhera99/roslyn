...
namespace Roslyn.VisualStudio.NewIntegrationTests.InProcess
{
    ...
    internal partial class CallHierarchyWindowInProcess
    {
        ...
        public async Task<ImmutableArray<ITableEntryHandle2>> GetContentsAsync(CancellationToken cancellationToken)
        {
            ...
            await TestServices.Workspace.WaitForAsyncOperationsAsync(FeatureAttribute.CallHierarchy, cancellationToken);

            // Find the tool window
            var tableControl = await GetCallHierarchyWindowAsync(cancellationToken);
        ...
        private async Task<IWpfTableControl2> GetCallHierarchyWindowAsync(CancellationToken cancellationToken)
        {
            await JoinableTaskFactory.SwitchToMainThreadAsync(cancellationToken);

            var shell = await GetRequiredGlobalServiceAsync<SVsUIShell, IVsUIShell>(cancellationToken);
            ErrorHandler.ThrowOnFailure(shell.FindToolWindowEx((uint)__VSFINDTOOLWIN.FTW_fFindFirst, ViewCallHierarchyGuid, dwToolWinId: 0, out var windowFrame));
